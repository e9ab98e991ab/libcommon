...
/**
 * @author gaoxin 18-12-20 下午8:37
 * @version V1.0.0
 * @name SpannableStringUtils 相关工具类 快速设置字体大小颜色 文体叠加
 * 示例
 * SpannableStringUtils
 *  .getBuilder("￥").setBold()
 *  .append(pay.getAmountInt()).setBold()
 *  .append(".").setBold()
 *  .append(pay.getAmountDouble()).setBold()
 *  .create());
 *
 */
public class SpannableStringUtils {
    ...
    public static class Builder {
        ...
        private void setSpan() {
            ...
            if (fontSize != -1) {
                mBuilder.setSpan(new AbsoluteSizeSpan(fontSize, true), start, end, flag);
                fontSize = -1;
            }
            if (xProportion != -1) {
